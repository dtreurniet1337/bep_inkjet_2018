...
- Y position changed from 5 to 8 inch. More gives less chance that paper folds
...
def p_raster_nxm(**kwargs):
    ...
    raster = b''
    for k in range(n):
        raster += (ESC_dollar(hor, (x + dx * k + ox)) + ESC_i_nrs(nozzlelist, color, size)) * rep
    
    # #doesnt work
    # yd = y * 2
    # rasterdata = ESC_v(pmgmt, y) + raster + ESC_v(pmgmt, yd) + raster +  b'\x0c'

    # works
    rasterdata = ESC_v(pmgmt, y) + raster + ESC_v(pmgmt, 2*y) + raster +  b'\x0c'

    # ### NEW ###
    # raster1 = b''
    # raster2 = b''

    # #print(str(range(oxn)) + ' oxn')
    ...
    #     # print('dddy: ' + str(dddy))
    # for k in range(n):
    #     raster1 += (ESC_dollar(hor, (x + dx * k)) + ESC_i_nrs(nozzlelist, color, size)) * rep
    #     raster2 += (ESC_dollar(hor, (x + dx * (1.25*k)) + ESC_i_nrs(nozzlelist, color, size)) * rep
    #     # raster += (ESC_dollar(hor, (x + dx * k + k2*ox)) + ESC_i_nrs(nozzlelist, color, size)) * rep + ESC_v(pmgmt,y+dddy)
    
    # #print("Raster created")  
  
    # rasterdata = ESC_v(pmgmt, y) + raster1 + ESC_v(pmgmt, y*2) + raster2 + b'\x0c'
    print("End p_raster_nxm()")
    return rasterdata
...
lpname_var.set("Epson-Stylus-XP235") #printer links
...
def get_values(event=None):
    ...
    ox=0 # x offset aanpassen
    oy=0 # y offset aanpassen #nog niet gebruikt
