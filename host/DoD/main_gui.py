...
- incorporated Arduino code in here
- default color black

Future todo's
- Further cleaning of code
- Incorporating line functions into manuals
...
from DoD.esc_functions import *
from DoD.hex_functions import *
from DoD.logos import *
...
def p_raster_nxm_diag(**kwargs): # Abel addition to draw lines
    global stepper_enable
    line_diag()
    stepper_enable = 1
    ...
    x_add = x + xlist[n_steps_n] / (2.54*10000) # adds dx to x base value
...
def printVar(variable, variableintext):
    print(variableintext + " is " + str(variable))
...
def line_diag (): # Abel addition to draw lines
    global xlist, ylist, xylist
    global x1, y1, x2, y2, Dx, Dy
    global d_overlap, d_overlap_y #use it for single step of stepper
    global n_steps_round

    ### Variables ###
    # # manual
    # x1 = 0
    # # print( "dx1: " + str(x1) + " micron.")
    # y1 = 0

    # x2 = 200 #for vertical lines use x1+1
    # y2 = 400
    ...
    x2 = dx2 #for vertical lines use x1+1
    if x1 == x2: # Not pretty
        x2 = x1 + 1/10000000
    y2 = dy2
    if y1 == y2:
    ...
    # d = 80 #micron
    d = d_drop
    d_overlap = 0.5*d # overlap on a straight line
    ...
    n_steps_round = math.ceil(n_steps) + 1 # +1 ensures it always at least passes the end

    print("Y-step " + str(n_steps_n+1) + "/" + str(n_steps_round))
    ...
    ### Usage ###
    #y=ax+b -> x=(y-b)/a. Different formulas of course possible
    xlist = []
    ylist = []
    for k in range(n_steps_round):
        y= k*d_overlap_y + y1
        ylist.insert(k,y)
...
def load_patterns(event=None):
    ...
    patterns = {
        ...
        'nxm raster' : {'posx':     [True, 166000],
                    ...
                    'rep':          [True,1],
                    'stretch':      [False,0],
                    'color' :       [False,'magenta'], #changing this won't change anything.
        ...
        'diagonal line' : {'posx':     [True, 166000],
                    'posy':         [True,8],
                    'dx':           [True, 250],
                    'dy':           [True, 0],
                    'rdx':          [False, 0],
...
tk.Tk.wm_title(root, "ESC/P2 Control Client (for Drop-on-demand)") #currently not true DoD as default nxm is not 1x1
...
def updatePatternParameters():
    ...
    addEntry(frame_patterns_par, 'dx1 (from DoD)', 'micron', patternDict['dx1'][1], nR(), 3, valcmd=valint) if patterns[patternSelected]['dx1'][0] else None
    addEntry(frame_patterns_par, 'dx2 (from DoD)', 'micron', patternDict['dx2'][1], nR(), 3, valcmd=valint) if patterns[patternSelected]['dx2'][0] else None
    addEntry(frame_patterns_par, 'dy1 (from DoD)', 'micron', patternDict['dy1'][1], nR(), 3, valcmd=valint) if patterns[patternSelected]['dy1'][0] else None
    addEntry(frame_patterns_par, 'dy2 (from DoD)', 'micron', patternDict['dy2'][1], nR(), 3, valcmd=valint) if patterns[patternSelected]['dy2'][0] else None
    addEntry(frame_patterns_par, 'Drop diameter', 'micron', patternDict['d_drop'][1], nR(), 3, valcmd=valint) if patterns[patternSelected]['d_drop'][0] else None
...
colorSelection.set('black') # this is where color is set
...
def get_values(event=None):
    ...
    #diag_line
    dx1 = patternDict['dx1'][1].get() if patternDict['dx1'][0] else 0
    dx2 = patternDict['dx2'][1].get() if patternDict['dx2'][0] else 0
    dy1 = patternDict['dy1'][1].get() if patternDict['dy1'][0] else 0
    dy2 = patternDict['dy2'][1].get() if patternDict['dy2'][0] else 0
    d_drop = patternDict['d_drop'][1].get() if patternDict['d_drop'][0] else 0
...
def run_program(event=None):
    ...
    header = load_prn_file('DoD/prns/' + prnname + '/' + prnname + '-header.prn')
    footer = load_prn_file('DoD/prns/' + prnname + '/' + prnname + '-footer.prn')
    ...
    try:
        ...
        # print("rasterdata obtained")

        #check if rasterdata is defined
        try:
            rasterdata
            # print("rasterdata defined")
        except NameError:
...
def print_esc_commands(event=None, PLNAME='def'):
    global n_steps_n, n_steps_round
    global stepper_enable
    global fatalError
